...
const PhaseController = dynamic(() => import('./education/education-phase-controller'), {
  ssr: false,
  loading: () => (
...
const SubHeading = ({ title, accent }: { title: string; accent: string }) => (
  ...
  >
    ...
    <div className="mt-2 h-0.5 w-10 rounded-full mx-auto lg:mx-0" style={{ background: accent }} />
  </motion.div>
);
...
const GrowthCommunity = () => {
  return (
    <section id={SECTION_IDS.growth} className="relative py-24 sm:py-32 overflow-hidden">
      <div className="pointer-events-none absolute inset-0 dot-grid opacity-15" aria-hidden="true" />

      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6">
        <SectionHeading
          title="Journey & Impact"
          subtitle="From classrooms to codebases — my learning path and developer footprint"
          accent="#d4a44a"
        />

        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 lg:gap-12 items-start">
          {/* Left column — Education Book */}
          <div className="min-w-0">
            <SubHeading title="Education" accent="#c9a961" />
            <PhaseController />
          </div>

          {/* Right column — Online Presence */}
          <div className="lg:sticky lg:top-24">
            <SubHeading title="Online Presence" accent="#EF9F27" />
            <PlatformsContent />
          </div>
        </div>
      </div>
