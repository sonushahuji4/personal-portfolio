'use client';

import dynamic from 'next/dynamic';
import SectionHeading from '@/components/ui/section-heading';
import { SECTION_IDS } from '@/lib/constants';

const PhaseController = dynamic(() => import('./education-phase-controller'), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center py-20">
      <div className="h-8 w-8 mx-auto rounded-full border-2 border-accent/30 border-t-accent animate-spin" />
    </div>
  ),
});

const Education = () => {
  return (
    <section id={SECTION_IDS.education} className="relative py-24 sm:py-32 overflow-hidden">
      <div className="relative z-10 mx-auto max-w-4xl px-6">
        <SectionHeading title="Education" subtitle="Flip through the pages of my academic journey" accent="#c9a961" />
        <PhaseController />
      </div>
    </section>
  );
};

export default Education;
